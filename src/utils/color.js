export default {
  ...
  isColor (colorx) {
    ...
    return prcolors.includes(colorx)
  },
  RandomColor () {
  ...
  rColor (colorx, opacity = 1) {
    ...
    if (colorx) {
      if (/[#()]/.test(colorx)) {
        ...
        if (prcolors.includes(colorx)) {
          return `rgba(var(--pr-${colorx}),${opacity})`
        } else {
          return `rgba(var(--pr-primary),${opacity})`
        }
      }
    } else {
      return `rgba(var(--pr-primary),${opacity})`
  ...
  changeColor (colorInicial) {
    ...
    if (colores.includes(colorInicial)) {
      ...
      colorx = this.getVariable(style, '--pr-' + colorInicial)
    } else {
      if (/[rgb()]/g.test(colorInicial)) {
        ...
        colorx = '--pr-' + colorInicial
      }
    }
    return colorx
    // this.setCssVariable('--pr-'+clave,colorx)
  }
}
